    ...
    const handleGeneratePDF = async () => {
        ...
        try {
            ...
            const checkPageBreak = (spaceNeeded: number) => {
                if (y + spaceNeeded > 280) { doc.addPage(); y = 20; return true; }
                return false;
            };

            doc.setFontSize(20); doc.setTextColor(50); doc.text("INFORME DUSOLAI", 20, y); y += 20;
            
            doc.setFontSize(14); doc.setTextColor(0); doc.text("1. RESUMEN EJECUTIVO", 20, y); y += 10;
            doc.setFontSize(10); doc.setTextColor(60);
            const summary = doc.splitTextToSize(cleanText(aiData.executive_summary), 170);
            doc.text(summary, 20, y); y += (summary.length * 5) + 10;

            checkPageBreak(50);
            doc.setFontSize(14); doc.setTextColor(0); doc.text("2. AUDITORIA TAREAS", 20, y); y += 10;
        ...
        } catch (e) { console.error(e); alert("Error PDF"); } finally { setIsGeneratingReport(false); }
    };

    return (
        <div className="flex flex-col h-screen bg-gray-50 dark:bg-gray-950 transition-colors duration-300 overflow-hidden">
            <header className="flex-shrink-0 bg-white dark:bg-gray-900 shadow-sm border-b border-gray-200 dark:border-gray-800 z-20">
                <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            <select value={currentWorkspaceId || ''} onChange={(e) => setWorkspace(e.target.value)} className="bg-gray-100 dark:bg-gray-800 border-none rounded-lg text-sm font-medium text-gray-700 dark:text-gray-200 p-2 outline-none">
